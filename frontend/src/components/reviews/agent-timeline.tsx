...
const statusConfig = {
  pending: { icon: Clock, className: "text-muted-foreground", bg: "bg-secondary/40 border-border" },
  running: { icon: Loader2, className: "text-primary animate-spin", bg: "bg-primary/10 border-primary/30" },
...
function AgentNode({ agent, isParallel = false }: { agent: AgentProgress; isParallel?: boolean }) {
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-4 p-4 rounded-xl border bg-card relative z-10 transition-colors duration-200",
        agent.status === "running" ? "border-primary/40" : "border-border",
        isParallel && "flex-1"
      )}
    >
